...
## Lab Objective
The objective of this lab is to demonstrate the Ansible ansible.builtin.copy module. The ansible.builtin.copy module copies a file from the local (controller) or remote host machine(s) to a location on the remote machine(s). If you want to harvest files from remote hosts, use the fetch module to return copies to the local box (controller).
